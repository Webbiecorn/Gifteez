      ...
      <div className="py-8 md:py-12">
        ...
        {personaQuickScan.length > 0 && (
          <section className="mb-12 rounded-3xl border border-gray-100 bg-white/80 p-6 shadow-sm">
            ...
            <div className="mt-6 flex overflow-x-auto pb-4 gap-4 snap-x snap-mandatory md:grid md:grid-cols-2 xl:grid-cols-4 md:overflow-visible md:pb-0 scrollbar-hide">
              {personaQuickScan.map((persona) => {
                const actionBadges = getQuickScanActionBadges(persona.action)
                return (
                  <article
                    key={persona.id}
                    className="min-w-[85vw] sm:min-w-[300px] md:min-w-0 snap-center flex h-full flex-col gap-4 rounded-2xl border border-gray-100 bg-white/70 p-5 shadow-[0_20px_50px_-40px_rgba(15,23,42,0.55)]"
                  >
                    <div className="space-y-2">
