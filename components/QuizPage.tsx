...
const relationshipCopy: Record<RelationshipType, string> = {
  partner: 'Voor je partner werken cadeaus met een romantische twist en persoonlijke touch extra goed.',
  friend: 'Voor je vriend(in) scoren verrassingen met fun-factor en gezamenlijke beleving het beste.',
  colleague: 'Voor een collega kies je iets luchtigs en praktisch dat ook op kantoor in de smaak valt.',
  family: 'Voor familie maak je indruk met warme cadeaus die herinneringen ophalen of samen momenten creëren.',
};
...
const resolveQuizOutcome = (
  ...
  const counts = personaKeys.reduce<Record<PersonaKey, number>>((acc, key) => {
    acc[key] = 0;
    return acc;
...
const QuizPage: React.FC<QuizPageProps> = ({ navigateTo }) => {
  ...
  const currentAnswerOption = currentQuestion?.answers.find(option => option.value === currentAnswerValue);
  const currentPersonaKey = currentAnswerOption?.resultKey && isPersonaKey(currentAnswerOption.resultKey) ? currentAnswerOption.resultKey : undefined;
  ...
  const questionSummaries = useMemo(
    () =>
      quizQuestions.map((question, index) => {
        ...
        const personaKey = answerOption?.resultKey && isPersonaKey(answerOption.resultKey) ? answerOption.resultKey : null;
      ...
      <>
        ...
        <section className="relative overflow-hidden bg-gradient-to-br from-orange-50 via-white to-pink-100 text-primary">
          <div className="pointer-events-none absolute inset-0">
            ...
            <div className="absolute top-1/3 right-0 h-64 w-64 rounded-full bg-amber-200/40 blur-[120px]" aria-hidden="true" />
            <div className="absolute bottom-0 left-1/3 h-80 w-80 rounded-full bg-rose-100/40 blur-3xl" aria-hidden="true" />
          </div>
              ...
              <div className="space-y-10">
                <div>
                  ...
                  <p className="mt-6 max-w-2xl text-lg text-primary/80">
                    Beantwoord drie slimme vragen en krijg direct een persoonlijk cadeau-profiel, inclusief topinteresses en inspiratie. Binnen een minuut weet je precies wat wel én niet werkt voor jouw ontvanger.
                  </p>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {personaKeys.map(key => {
                    ...
                    return (
                      ...
                      >
                        <div className={`flex h-12 w-12 items-center justify-center rounded-xl bg-white shadow ${meta.accent}`}>
                          <Icon className="h-6 w-6" />
                        </div>
                        <div className="space-y-1">
                          <p className="text-sm font-semibold uppercase tracking-wide text-primary/70">{meta.badge}</p>
                          <p className="text-base font-semibold text-primary">{quizResults[key]?.title}</p>
              ...
              {[
                {
                  ...
                  description: 'Elke vraag is ontworpen om snel voorkeuren scherp te krijgen zonder lange lijsten.',
                },
                {
                  title: 'Ontvang een profiel',
                  description: 'We matchen je antwoorden met één van de vier cadeau persoonlijkheden.',
                },
                {
                  title: 'Krijg direct inspiratie',
                  description: 'Met concrete interesses, blogtips en een doorklik naar de AI GiftFinder.',
                },
              ].map(stepItem => (
                <div key={stepItem.title} className="rounded-2xl border border-primary/10 bg-gradient-to-br from-white via-white to-rose-50 p-6 shadow-sm">
                  <p className="text-sm font-semibold uppercase tracking-wide text-primary/60">{stepItem.title}</p>
          ...
          <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-col items-start gap-2 md:items-end">
              ...
              <div className="flex items-baseline gap-2">
                <span className="font-display text-3xl font-bold text-accent">{progressPercentage}%</span>
          ...
          <div className="grid gap-8 lg:grid-cols-[1.7fr,1fr]">
            <div className="space-y-6 rounded-3xl border border-white/80 bg-white/95 p-8 shadow-xl">
              ...
              <div className="space-y-4">
                {currentQuestion.answers.map((answer, index) => {
                  const personaKey = answer.resultKey && isPersonaKey(answer.resultKey) ? answer.resultKey : undefined;
                  const persona = personaKey ? personaMeta[personaKey] : undefined;
                  const personaResult = personaKey ? quizResults[personaKey] : undefined;
                  const metaConfig = currentQuestion.metaKey ? metaQuestionMeta[currentQuestion.metaKey] : undefined;
                    ...
                    >
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6">
                        ...
                        <div className="flex-1 space-y-2">
                          <p className={`text-lg font-semibold leading-relaxed text-primary ${isSelected ? 'text-accent' : ''}`}>
                            {answer.text}
                          </p>
                          {persona && personaResult ? (
                            <div className="flex flex-wrap items-center gap-2 text-sm">
                              <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1 font-semibold ${persona.accentSoft} ${persona.accent}`}>
                            ...
                            (answer.helperText || metaConfig) && (
                              <div className="space-y-2 text-sm text-primary/65">
                                {metaConfig && (
                                  <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1 font-semibold ${metaConfig.accentSoft} ${metaConfig.accent}`}>
                                    <Icon className="h-4 w-4" />
                                    {metaConfig.label}
                                  </span>
                                )}
                        ...
                        <div className={`transition-opacity duration-300 ${isSelected ? 'opacity-100' : 'opacity-0 sm:opacity-0 sm:group-hover:opacity-100'}`}>
            ...
            <aside className="space-y-6">
              <div className="rounded-3xl border border-white/70 bg-white/95 p-6 shadow-lg">
                ...
                <div className="mt-4 space-y-3">
                  {questionSummaries.map(summary => {
                    ...
                    const personaStyle = summary.personaKey ? personaMeta[summary.personaKey] : undefined;
                    const metaConfig = summary.question.metaKey ? metaQuestionMeta[summary.question.metaKey] : undefined;
                    const Icon = isPersonaQuestion && personaStyle ? personaStyle.icon : metaConfig?.icon ?? CheckIcon;
                    const iconColorClass = isPersonaQuestion && personaStyle ? personaStyle.accent : metaConfig?.accent ?? 'text-primary/30';
                    ...
                    return (
                      <button
                        ...
                        className={`flex w-full items-start gap-3 rounded-2xl border px-4 py-3 text-left transition ${
                          isActive ? 'border-accent/50 bg-accent/5' : 'border-primary/10 bg-white/90 hover:border-accent/40'
                        }`}
                      >
                        <span className={`mt-0.5 inline-flex h-7 w-7 items-center justify-center rounded-full text-sm font-semibold ${
                          summary.personaKey ? personaMeta[summary.personaKey].accentSoft : 'bg-primary/10'
                        } ${summary.personaKey ? personaMeta[summary.personaKey].accent : 'text-primary'}`}>
                          {summary.index + 1}
                        </span>
                        <div className="flex-1 text-sm">
                          <p className="font-semibold text-primary/80 line-clamp-2">{summary.question.text}</p>
              ...
              {currentPersonaMeta && currentPersonaResult && (
                <div className={`rounded-3xl border border-white/70 bg-white/95 p-6 shadow-lg ${currentPersonaMeta.accentSoft}`}>
                  <div className="flex items-center gap-3">
                    <currentPersonaMeta.icon className={`h-6 w-6 ${currentPersonaMeta.accent}`} />
                    <div>
                      <p className="text-sm font-semibold uppercase tracking-wide text-primary/60">Voorproefje</p>
                      <p className="font-display text-xl text-primary">{currentPersonaResult.title}</p>
                    </div>
                  </div>
  ...
  if (step === 'result' && result && resultKey) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-light-bg via-white to-rose-50/70 pb-24">
        <section className={`relative overflow-hidden bg-gradient-to-br ${persona.gradient} text-primary`}>
          <div className="pointer-events-none absolute inset-0">
            <div className="absolute -top-10 left-0 h-72 w-72 rounded-full bg-white/40 blur-3xl" aria-hidden="true" />
            <div className="absolute top-1/4 right-0 h-64 w-64 rounded-full bg-white/30 blur-[120px]" aria-hidden="true" />
            <div className="absolute bottom-0 left-1/3 h-80 w-80 rounded-full bg-white/30 blur-3xl" aria-hidden="true" />
          </div>
                ...
                <Button
                  ...
                  onClick={() =>
                    navigateTo('giftFinder', {
                      persona: resultKey,
                      budget: budgetValue,
                      relationship: relationshipValue ? relationshipLabels[relationshipValue] : undefined,
        ...
        <div className="container mx-auto px-4 py-16 sm:px-6 lg:px-8">
          <div className="grid gap-8 lg:grid-cols-[2fr,1fr]">
            <div className="space-y-8">
              <div className="rounded-3xl border border-white/70 bg-white/95 p-8 shadow-xl">
                <div className="mb-6 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold uppercase tracking-wide text-primary/60">Persoonlijkheidsinsights</p>
                    <p className="font-display text-2xl text-primary">Wat werkt voor {result.title.toLowerCase()}</p>
                  </div>
                </div>
                ...
                <div className="mt-6 rounded-2xl border border-primary/10 bg-gradient-to-r from-white via-white to-rose-50 p-6">
                  <p className="text-sm font-semibold uppercase tracking-wide text-primary/60">Snelle tip</p>
              ...
              <div className="rounded-3xl border border-white/70 bg-white/95 p-8 shadow-xl">
                <div className="mb-6 flex items-center gap-3">
                  ...
                  <h2 className="font-display text-2xl text-primary">Shopping list voor jouw budget</h2>
                </div>
                {shoppingList && shoppingList.length > 0 ? (
                  <div className="grid gap-6 md:grid-cols-2">
                    {shoppingList.map(item => (
                      <div key={item.title} className="flex h-full flex-col justify-between rounded-2xl border border-primary/10 bg-white/90 p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-wide text-primary/50">Curated pick</p>
              ...
              <div className="rounded-3xl border border-white/70 bg-white/95 p-8 shadow-xl">
                ...
                {relatedBlogs.length > 0 ? (
                  ...
                  <p className="text-primary/70">
                    Momenteel hebben we geen blogartikelen gekoppeld aan dit profiel. Kom snel terug voor nieuwe inspiratie!
                  </p>
                )}
              </div>
            </div>
            <aside className="space-y-6">
              <div className="rounded-3xl border border-white/70 bg-white/95 p-6 shadow-xl">
                ...
                <div className="mt-5">
                  <QuizShareCard
                    ...
                    relationshipLabel={relationshipValue ? relationshipLabels[relationshipValue] : null}
                  />
                </div>
              </div>
              ...
              <div className="rounded-3xl border border-white/70 bg-white/95 p-6 shadow-xl">
                ...
                <div className="space-y-3">
                  {questionSummaries.map(summary => {
                    ...
                    const metaConfig = !isPersonaQuestion && summary.question.metaKey ? metaQuestionMeta[summary.question.metaKey] : null;
                    const Icon = isPersonaQuestion && personaData ? personaData.icon : metaConfig?.icon ?? SparklesIcon;
                    const badgeClasses = isPersonaQuestion && personaData
                    ...
                    return (
                      <div key={summary.question.id} className="rounded-2xl border border-primary/10 bg-white/95 p-4 shadow-sm">
                        <div className="flex items-start gap-3">
                          <span className={`mt-0.5 inline-flex h-7 w-7 items-center justify-center rounded-full text-sm font-semibold ${badgeClasses}`}>
                            {summary.index + 1}
                          </span>
                          <div className="flex-1 text-sm">
                            <p className="font-semibold text-primary/80 line-clamp-2">{summary.question.text}</p>
                            <div className="mt-2 flex items-center gap-2 text-primary/70">
                              <Icon className={`h-4 w-4 ${isPersonaQuestion && personaData ? personaData.accent : metaConfig?.accent ?? 'text-primary/50'}`} />
              ...
              <div className="rounded-3xl border border-white/70 bg-white/95 p-6 shadow-xl">
                <p className="text-sm font-semibold uppercase tracking-wide text-primary/60">Volgende stap</p>
                <p className="mt-2 text-primary/80">
                  Klaar om concrete cadeau-ideeën te krijgen? Gebruik je profiel in de AI GiftFinder en filter direct op interesses als
                  <span className="font-semibold text-primary"> {result.recommendedInterests.toLowerCase()}</span>.
                </p>
