...
import { SearchIcon, BookOpenIcon, CalendarIcon, UserIcon, SparklesIcon, MenuIcon, TargetIcon, TagIcon } from './IconComponents';
...
const SpotlightHeroCard: React.FC<{ post: BlogPost; navigateTo: NavigateTo; }> = ({ post, navigateTo }) => {
...
const SpotlightSupportCard: React.FC<{ post: BlogPost; navigateTo: NavigateTo; index: number; }> = ({ post, navigateTo, index }) => {
    const formattedDate = new Date(post.publishedDate).toLocaleDateString('nl-NL', {
        year: 'numeric', month: 'long', day: 'numeric'
    });
    const readingTime = getReadingTime(post.excerpt);
...
const ArchiveListItem: React.FC<{ post: BlogPost; navigateTo: NavigateTo; }> = ({ post, navigateTo }) => {
    const formattedDate = new Date(post.publishedDate).toLocaleDateString('nl-NL', {
    ...
    return (
        <li className="relative pl-10">
            ...
            <button
                ...
                className="text-left"
            >
                <span className="text-sm font-semibold uppercase tracking-wide text-accent/80">{post.category}</span>
...
const BlogCard: React.FC<{ post: BlogPost; navigateTo: NavigateTo; isFeatured?: boolean; }> = ({ post, navigateTo, isFeatured = false }) => {
    ...
    return (
        ...
        >
            <div className="relative overflow-hidden" onClick={() => navigateTo('blogDetail', { slug: post.slug })}>
                <ImageWithFallback
                    src={post.imageUrl}
                    alt={post.title}
            ...
            <div className={`flex flex-col p-6 md:p-7 ${isFeatured ? 'md:p-9' : ''}`}>
                <h3 className={`font-display font-bold text-primary leading-tight transition-colors duration-300 group-hover:text-accent ${isFeatured ? 'text-xl md:text-2xl' : 'text-lg'}`}>
                    <button
                        onClick={() => navigateTo('blogDetail', { slug: post.slug })}
                        className="text-left"
                    >
                ...
                <p className={`mt-3 text-gray-600 leading-relaxed ${isFeatured ? 'text-base' : 'text-sm line-clamp-4'}`}>
                    {post.excerpt}
                </p>
...
const BlogPage: React.FC<{ navigateTo: NavigateTo; }> = ({ navigateTo }) => {
    ...
    const filteredPosts = useMemo(() => {
        return otherPosts
            ...
            .filter(
                (post) =>
                    selectedYear === 'All' || new Date(post.publishedDate).getFullYear().toString() === selectedYear
            )
            .filter(
                (post) =>
    ...
    const isFilteringActive = selectedCategory !== 'All' || selectedAuthor !== 'All' || selectedYear !== 'All' || searchQuery.trim() !== '';
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-white via-muted-rose/40 to-white">
            ...
            <section className="relative overflow-hidden">
                ...
                <Container size="xl" className="relative z-10 py-20 md:py-24">
                        ...
                        <div className="space-y-8 text-center lg:text-left">
                            ...
                            <div className="space-y-4">
                                ...
                                <p className="typo-lead mx-auto max-w-2xl text-slate-600 lg:mx-0">
                                    Jouw bron voor inspiratie, tips en de beste cadeau-ideeën voor elke gelegenheid. Ontdek onze nieuwste cadeaugidsen en tover een glimlach op elk gezicht.
                                </p>
                            </div>
                        ...
                        {featuredPost ? (
                            <article className="group relative h-full overflow-hidden rounded-[40px] border border-white/10 bg-slate-900/80 shadow-[0_50px_100px_-60px_rgba(15,23,42,0.7)] backdrop-blur">
                                ...
                                <div className="relative flex h-full flex-col justify-end gap-6 p-8 md:p-10">
                                    <div className="flex flex-wrap items-center gap-2 text-xs font-semibold text-white/80">
                                        ...
                                        <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 backdrop-blur">
                                            <CalendarIcon className="h-3 w-3" />
                                            {new Date(featuredPost.publishedDate).toLocaleDateString('nl-NL', { year: 'numeric', month: 'long', day: 'numeric' })}
                                        </span>
                                        <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 backdrop-blur">
                                            <BookOpenIcon className="h-3 w-3" />
                                    ...
                                    <div className="space-y-3">
                                        <p className="text-sm font-semibold uppercase tracking-[0.3em] text-white/50">Uitgelicht</p>
                            ...
                            <div className="relative rounded-[36px] border border-white/30 bg-white/60 p-8 shadow-xl shadow-white/30 backdrop-blur">
                                <div className="space-y-4 text-center lg:text-left">
                                    <h2 className="font-display text-2xl font-bold text-primary">Publiceer je eerste gids</h2>
                                    <p className="text-sm text-gray-600">
                                        Zodra de eerste blog live staat, verschijnt hier een uitgelichte cadeaugids met tips en highlights.
                                    </p>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="mt-12 grid grid-cols-2 gap-4 text-left sm:grid-cols-4">
                        {[{ label: 'Cadeaugidsen', value: publishedPosts.length }, { label: 'Categorieën', value: categories.length - 1 }, { label: 'Experts', value: authors.length - 1 }, { label: 'Inspiratie', value: '24/7' }].map((stat) => (
                            <div key={stat.label} className="rounded-2xl border border-white/60 bg-white/80 p-5 text-slate-700 shadow-[0_20px_60px_-50px_rgba(15,23,42,0.75)] backdrop-blur">
            ...
            <Container size="xl" className="py-16 space-y-16">
                {!isFilteringActive && (
                    <section className="rounded-[40px] bg-white/80 p-8 md:p-10 shadow-[0_45px_120px_-80px_rgba(15,23,42,0.35)] ring-1 ring-white/60 backdrop-blur">
                        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                            <div>
                                <h2 className="font-display text-2xl md:text-3xl font-bold text-primary">Uitgelichte cadeaugidsen</h2>
                                <p className="max-w-2xl text-gray-600">Een handpicked selectie van gidsen die vandaag in de schijnwerpers staan.</p>
                            </div>
                        ...
                        {spotlightPrimaryPost ? (
                                ...
                                <div className="flex flex-col gap-6">
                                    {supportingSpotlightPosts.length > 0 ? (
                                        supportingSpotlightPosts.map((post, index) => (
                                            <SpotlightSupportCard key={post.slug} post={post} index={index} navigateTo={navigateTo} />
                                        ))
                                    ) : (
                                        <div className="flex h-full items-center justify-center rounded-3xl border border-dashed border-muted-rose/70 bg-white/80 p-10 text-center shadow-inner backdrop-blur">
                                            <div className="space-y-3">
                                                ...
                                                <h3 className="font-display text-lg font-semibold text-primary">Nog meer inspiratie volgt</h3>
                                                <p className="text-sm text-gray-600">Publiceer extra artikelen om dit overzicht te vullen.</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="mt-10 rounded-[32px] border border-dashed border-muted-rose/80 bg-white/70 p-10 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.8)] backdrop-blur">
                                ...
                                <h2 className="font-display text-2xl font-bold text-primary">Nog geen extra gidsen</h2>
                                <p className="mt-2 text-gray-600">Zodra er meer artikelen live staan, vind je ze hier terug.</p>
                            </div>
                        )}
                    </section>
                )}
                ...
                {!isFilteringActive && categoryHighlights.length > 0 && (
                    <section className="rounded-[32px] bg-white/80 p-8 md:p-10 shadow-[0_40px_120px_-80px_rgba(15,23,42,0.35)] ring-1 ring-slate-100 backdrop-blur">
                        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                            <div>
                                <h2 className="font-display text-2xl md:text-3xl font-bold text-primary">Categorieën om te ontdekken</h2>
                                <p className="max-w-2xl text-gray-600">Kies snel een thema en wij tonen direct alle bijpassende cadeaugidsen.</p>
                            </div>
                            <div className="inline-flex items-center gap-2 rounded-full bg-accent/10 px-4 py-2 text-sm font-semibold text-accent">
                                <SparklesIcon className="h-4 w-4" />
                                Trending thema's
                        ...
                        <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                            {categoryHighlights.map(({ name, count }) => (
                                ...
                                >
                                    <div>
                                        <span className="text-xs font-semibold uppercase tracking-wide text-accent/80">Categorie</span>
                                        <h3 className="mt-2 font-display text-lg font-bold text-primary">{name}</h3>
                                        <p className="mt-1 text-sm text-gray-500">{count} {count === 1 ? 'gids' : 'gidsen'}</p>
                                    </div>
                ...
                <section id="blog-filters" className="rounded-[32px] bg-white p-6 md:p-8 shadow-[0_24px_80px_-60px_rgba(15,23,42,0.45)] ring-1 ring-slate-100">
                    <div className="flex flex-col gap-6">
                        <div className="flex flex-wrap items-center gap-3">
                ...
                <section className="space-y-8">
                    <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                        <div>
                            <h2 className="font-display text-2xl md:text-3xl font-bold text-primary">{resultsHeadline}</h2>
                    ...
                    {curatedGridPosts.length === 0 && (
                        <div className="rounded-3xl border border-dashed border-muted-rose/60 bg-white/60 p-12 text-center backdrop-blur">
                            ...
                            <p className="mt-1 text-sm text-gray-500">Pas je filters aan of wis ze om opnieuw te beginnen.</p>
                        </div>
                    )}
                </section>
                ...
                {!isFilteringActive && archivePosts.length > 0 && (
                    <section className="rounded-[32px] bg-gradient-to-br from-white via-white to-muted-rose/30 p-8 md:p-10 shadow-[0_40px_120px_-90px_rgba(244,63,94,0.35)] ring-1 ring-muted-rose/40">
                        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                            <div>
                                <h2 className="font-display text-2xl md:text-3xl font-bold text-primary">Uit het archief</h2>
                                <p className="max-w-2xl text-gray-600">Blijf ontdekken met eerdere cadeaugidsen die nog steeds boordevol inspiratie zitten.</p>
                            </div>
