...
const GiftFinderPage: React.FC<GiftFinderPageProps> = ({ initialData, showToast }) => {
  ...
  const handleFeedback = useCallback((isPositive: boolean) => {
    ...
    pinterestSearch(`feedback_${isPositive ? 'thumbs_up' : 'thumbs_down'}`, `feedback_${Date.now()}`);
    
    showToast(isPositive ? '👍 Bedankt voor je feedback!' : '👎 We zullen onze suggesties verbeteren!');
  ...
  setFeedbackSubmitting(false);
    ...
    try {
      ...
      const resultsWithReasons = enhancedResults.map(gift => {
        ...
        if (currentMonth === 9 && gift.tags?.some(tag => tag.toLowerCase().includes('halloween'))) {
          trendingBadge = 'seasonal';
        } else if ([10, 11].includes(currentMonth) && gift.tags?.some(tag => 
        ...
        if (occasion && gift.tags?.some(tag => tag.toLowerCase().includes(occasion.toLowerCase()))) {
      ...
      setTimeout(() => {
        document.getElementById('gift-results')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 300);
    } catch (err: any) {
          ...
          <form onSubmit={handleSubmit} className="bg-white p-8 md:p-12 rounded-2xl shadow-2xl border border-gray-100 space-y-10 -mt-16 relative z-20">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-blue-500/10 rounded-lg"><span className="text-2xl">🎨</span></div>
                <div className="flex-1">
                  <label htmlFor="interests" className="block font-display text-xl font-bold text-primary mb-2">Hobby's of interesses?</label>
                  <p className="text-sm text-gray-600">Selecteer interesses of voeg je eigen toe</p>
                </div>
              </div>
              ...
              <div className="relative">
                <label htmlFor="interests" className="block text-sm font-medium text-gray-700 mb-2">Of voeg eigen interesses toe:</label>
                <input
                  id="interests"
                  type="text"
                  value={interests}
            ...
            {!isLoading && !error && searchPerformed && gifts.length === 0 && (
              noAffiliateResults ? (
                <div className="text-center py-12 bg-gradient-to-br from-secondary/10 via-white to-primary/5 border border-secondary/30 rounded-2xl">
                  ...
                  <h3 className="mt-6 font-display text-2xl font-bold text-primary">Onze partners hebben hier nog niets voor.</h3>
                  <p className="mt-3 text-gray-600 max-w-xl mx-auto">
                    We tonen alleen cadeaus met affiliate commissie. Probeer een andere combinatie of ga direct naar onze favoriete winkels hieronder.
                  </p>
                  ...
                  <p className="mt-4 text-xs text-gray-500">Links openen in een nieuw tabblad met affiliate tracking.</p>
                  <div className="mt-8 max-w-xl mx-auto bg-white border border-secondary/30 rounded-2xl p-6 shadow-lg text-left">
                    <h4 className="font-display text-lg font-bold text-primary mb-2">📬 Ontvang exclusieve cadeaudeals</h4>
                    <p className="text-gray-600 text-sm mb-4">
                      Schrijf je in voor een korte wekelijkse update met Amazon & Coolblue topdeals en ontvang direct onze gratis Cadeau Planner.
                    </p>
                    <a
                      href="/download"
            ...
            {gifts.length > 0 && (
              <div className="animate-fade-in">
                ...
                {personalizedIntro && (
                  <div className="mb-8 p-6 bg-gradient-to-r from-primary/10 via-accent/10 to-secondary/20 rounded-xl border-2 border-primary/20 shadow-lg">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <h3 className="font-display text-lg font-bold text-primary mb-2">Jouw Persoonlijke AI Assistent</h3>
                ...
                {showCompareView && compareList.length > 0 && (
                  <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowCompareView(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                      <div className="sticky top-0 bg-white border-b border-gray-200 p-6 flex items-center justify-between z-10">
                        <h2 className="font-display text-2xl font-bold text-primary">Vergelijk Cadeaus</h2>
                        <button onClick={() => setShowCompareView(false)} className="p-2 hover:bg-gray-100 rounded-full transition-colors">
                          <span className="text-2xl">✕</span>
                        </button>
                      </div>
                      <div className="p-6">
                        <div className={`grid ${compareList.length === 2 ? 'grid-cols-2' : 'grid-cols-3'} gap-6`}>
                          {compareList.map((gift, idx) => (
                            <div key={gift.productName} className="border border-gray-200 rounded-xl p-4 relative">
                              <button
                                onClick={() => handleToggleCompare(gift)}
                                className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-600"
                              >✕</button>
                              <GiftResultCard gift={gift} index={idx} showToast={showToast} isEmbedded candidateVariant />
                            </div>
                          ))}
                        </div>
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {gifts.slice(0, visibleCount).map((gift, index) => (
                    <GiftResultCard 
                      ...
                      isInCompareList={compareList.some(g => g.productName === gift.productName)}
                    />
                  ))}
                </div>
        ...
        <div className="fixed bottom-6 right-6 z-40">
          <div className="bg-gradient-to-r from-primary to-accent text-white rounded-full shadow-2xl px-6 py-4 flex items-center gap-4">
            ...
            >
              ...
              <span>Vergelijk {compareList.length} {compareList.length === 1 ? 'cadeau' : 'cadeaus'}</span>
            </button>
            <button
