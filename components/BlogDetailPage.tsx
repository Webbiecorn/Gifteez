...
const _BlogCardSmall: React.FC<{ post: BlogPost; navigateTo: NavigateTo }> = ({
  ...
  <div
    className="bg-white rounded-2xl shadow-lg overflow-hidden group cursor-pointer border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
  ...
  >
    ...
    <div className="p-4">
      <h4 className="font-display text-sm font-bold text-primary line-clamp-2 mb-2 group-hover:text-blue-600 transition-colors duration-300">
        {post.title}
      </h4>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-white via-muted-rose/30 to-white flex items-center justify-center px-4">
        <div className="max-w-lg bg-white border border-gray-100 shadow-xl rounded-3xl p-8 text-center space-y-4">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-rose-100 text-rose-600 text-2xl">
            😕
          </div>
          <h1 className="text-2xl font-bold text-gray-900">Blogpost niet gevonden</h1>
          <p className="text-gray-600">
            {fetchError ?? 'Het artikel waar je naar zocht bestaat niet meer of is verplaatst.'}
          </p>
    ...
    switch (block.type) {
      ...
      case 'image': {
        ...
        return (
          <figure key={index} className="my-12">
            <div
              className={`relative overflow-hidden rounded-3xl border border-gray-100 shadow-xl bg-white ${globalImageFit === 'cover' ? 'h-80 md:h-[26rem]' : 'p-4 md:p-6'}`}
            >
      ...
      case 'comparisonTable': {
        ...
        return (
          <div
            key={index}
            className="my-12 overflow-x-auto bg-white rounded-2xl shadow-lg border border-gray-100"
          >
            <table className="w-full text-sm text-left text-gray-500">
              <thead className="text-xs text-white uppercase bg-gradient-to-r from-accent to-accent-hover">
                <tr>
                  <th scope="col" className="px-8 py-4 font-bold text-white rounded-tl-2xl">
                    Specificatie
                  </th>
                  {tableBlock.headers.map((header, i) => (
                    <th
                      key={i}
                      scope="col"
                      className="px-8 py-4 font-bold text-white text-center"
                      style={{
                        background:
                          i === tableBlock.headers.length - 1
              ...
              <tbody>
                {tableBlock.rows.map((row, i) => (
                  <tr
                    key={i}
                    className={`border-b border-gray-100 ${i % 2 === 0 ? 'bg-gray-50' : 'bg-white'} hover:bg-muted-rose/70 transition-colors duration-200`}
                  >
                    <th
                      scope="row"
                      className="px-8 py-4 font-bold text-gray-900 whitespace-nowrap border-r border-gray-200 bg-gradient-to-r from-gray-100 to-gray-50"
                    >
                      {row.feature}
                    </th>
                    {row.values.map((value, j) => (
                      <td
                        key={j}
                        className="px-8 py-4 border-r border-gray-100 text-center font-medium text-gray-700 last:border-r-0"
                      >
                        {value}
                      </td>
                    ))}
      ...
      case 'verdict': {
        ...
        return (
          <div
            key={index}
            className="my-12 relative overflow-hidden bg-white border-2 border-muted-rose rounded-2xl shadow-xl"
          >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {heroDisplaySrc && (
          <section className="mb-12">
            ...
            >
              ...
              <div
                className={`absolute inset-0 pointer-events-none ${heroFitMode === 'cover' ? 'bg-gradient-to-t from-black/45 via-black/15 to-transparent' : 'bg-gradient-to-t from-black/20 via-black/5 to-transparent'}`}
              ></div>
              <div className="absolute top-6 left-6 flex flex-wrap items-center gap-3 text-sm font-semibold">
                <span className="bg-white/85 text-primary px-4 py-1.5 rounded-full shadow-lg backdrop-blur-sm">
                  {post.category}
                </span>
                <span className="bg-white/75 text-gray-700 px-4 py-1.5 rounded-full shadow-md backdrop-blur-sm">
                  {formattedDate}
                </span>
              </div>
        ...
        <nav aria-label="Breadcrumb" className="mb-8">
          <ol className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <li>
              <span className="font-medium text-gray-800 line-clamp-1" aria-current="page">
                {post.title}
              </span>
            </li>
          </ol>
          ...
          <main className="lg:col-span-8">
            <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
              ...
              <div className="p-8 md:p-12">
                ...
                {headings.length > 1 && (
                  <nav
                    className="mb-12 p-6 bg-gradient-to-r from-secondary to-muted-rose rounded-2xl border border-muted-rose"
                    aria-labelledby="toc-heading"
                  >
                    <div className="flex items-center gap-3 mb-4">
                      <SparklesIcon className="w-6 h-6 text-accent" aria-hidden="true" />
                      <h2 id="toc-heading" className="font-display font-bold text-primary text-xl">
                        In dit artikel
                      </h2>
                    </div>
                    <ol className="space-y-3" role="list">
                      {headings.map((heading, i) => (
                        <li key={i} className="flex items-center gap-2" role="listitem">
                          <span
                            className="w-6 h-6 bg-accent text-white rounded-full flex items-center justify-center text-xs font-bold"
                            aria-hidden="true"
                          >
                            {i + 1}
                          </span>
                          ...
                          <span id={`heading-${i}-description`} className="sr-only">
                            Spring naar sectie {heading.content}
                          </span>
                        </li>
                      ))}
                    </ol>
                ...
                <div className="flex flex-wrap items-center gap-4 mb-8 text-sm text-gray-600 border-b border-gray-100 pb-6">
                  <div className="flex items-center gap-2">
                    ...
                    <time
                      ...
                      aria-label={`Gepubliceerd op ${formattedDate}`}
                    >
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-yellow-700 text-sm leading-relaxed">
                        Sommige links in dit artikel zijn affiliate links. Als je via deze links
                        iets koopt, ontvangen wij een kleine commissie zonder extra kosten voor jou.
                        Dit helpt ons om gratis content te blijven maken. Onze mening blijft altijd
            ...
            >
              ...
              <div className="grid md:grid-cols-3 gap-4 mb-8">
                ...
                >
                  <svg
                    className="w-6 h-6 text-highlight group-hover:scale-110 transition-transform duration-300"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                <button
                  onClick={() =>
                    showToast('Bedankt voor je feedback! We verbeteren continu.', 'info')
                  }
                ...
                >
                  <svg
                    className="w-6 h-6 text-yellow-600 group-hover:scale-110 transition-transform duration-300"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                    ...
                    <ol className="space-y-3" role="list">
                      {headings.map((heading, i) => (
                        <li key={i} role="listitem">
                          ...
                          >
                            <span
                              className="w-6 h-6 bg-muted-rose text-accent rounded-full flex items-center justify-center text-xs font-bold group-hover:bg-accent group-hover:text-white transition-colors duration-300"
                              aria-hidden="true"
                            >
                              {i + 1}
                            </span>
                          ...
                          <span id={`sidebar-heading-${i}-description`} className="sr-only">
                            Spring naar sectie {heading.content}
                          </span>
                        </li>
                      ))}
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-100">
                    ...
                    <div className="space-y-2">
                      ...
                      <button
                        onClick={() =>
                          document
                            .querySelector('[data-author-section]')
                            ?.scrollIntoView({ behavior: 'smooth' })
                        }
                        className="w-full text-left px-3 py-2 text-sm text-gray-600 hover:text-accent hover:bg-muted-rose/70 rounded-lg transition-colors duration-200"
                      >
                ...
                .map((rel) => (
                  ...
                  >
                    ...
                    <div className="p-5 flex flex-col h-full">
                      <span className="inline-block text-[11px] font-semibold tracking-wide px-2 py-1 bg-muted-rose text-accent rounded-full mb-3 uppercase">
                        {rel.category}
                      </span>
                      <h4 className="font-display text-lg font-bold text-primary leading-snug line-clamp-2 mb-3">
                        {rel.title}
                      </h4>
