...
const DealCategoryManager: React.FC<DealCategoryManagerProps> = ({ onSaved }) => {
  ...
  const handleBulkAddToCategory = (categoryIndex: number) => {
    ...
    const newCategories = [...categories]
    const category = newCategories[categoryIndex]
    const newIds = Array.from(selectedProducts).filter(
      (id: string) => !category.itemIds.includes(id)
    )
    category.itemIds = [...category.itemIds, ...newIds] as string[]
    setCategories(newCategories)
    setMessage({
      type: 'success',
      text: `✅ ${newIds.length} product${newIds.length > 1 ? 'en' : ''} toegevoegd!`,
    })
  ...
  const handleBulkRemoveFromCategory = (categoryIndex: number) => {
    ...
    setMessage({
      type: 'success',
      text: `✅ ${idsToRemove.length} product${idsToRemove.length > 1 ? 'en' : ''} verwijderd!`,
    })
  ...
  const handleApplyTemplate = (
    templateCategories: CategoryBlockConfig[],
    mode: 'replace' | 'merge'
  ) => {
    if (mode === 'replace') {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                Sleep om volgorde te wijzigen • Voeg producten toe aan categorieën
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    {categories.map((category, categoryIndex) => (
                      <SortableCategory
                        ...
                        onRemoveProduct={(productId) =>
                          handleRemoveProductFromCategory(categoryIndex, productId)
                        }
                        onMoveProduct={(productIndex, direction) =>
                          handleMoveProduct(categoryIndex, productIndex, direction)
                        }
                      />
                    ))}
                  </div>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-semibold text-gray-900">
                    Beschikbare Producten ({allProducts.length})
                  </h3>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          setGiftScoreRange([parseFloat(e.target.value), giftScoreRange[1]])
                        }
                        className="flex-1"
                      />
                      <input
                        ...
                        onChange={(e) =>
                          setGiftScoreRange([giftScoreRange[0], parseFloat(e.target.value)])
                        }
                        className="flex-1"
                      />
                    </div>
                  ...
                  {loadingSuggestions ? (
                    ...
                    <div className="space-y-2 max-h-[400px] overflow-y-auto">
                      {suggestions.map((suggestion) => {
                        ...
                        const isInCategory = categories[selectedCategoryIndex].itemIds.includes(
                          product.id
                        )
              ...
              {selectedCategoryIndex === null ? (
                <div className="text-center p-12 border-2 border-dashed border-gray-300 rounded-lg">
                  <SparklesIcon className="h-12 w-12 mx-auto text-gray-400 mb-4" />
                  <p className="text-gray-600">
                    Selecteer een categorie om producten toe te voegen
                  </p>
                </div>
              ) : (
  ...
  return (
    <div
      ...
      className={`p-3 rounded-lg border-2 transition-all cursor-move ${
        isSelected ? 'border-rose-500 bg-rose-50' : 'border-gray-200 bg-white hover:border-gray-300'
                ...
                {category.itemIds.map((productId, productIndex) => {
                  ...
                  return (
                    <SortableProduct
                      ...
                      onMoveUp={
                        productIndex > 0 ? () => onMoveProduct(productIndex, 'up') : undefined
                      }
                      onMoveDown={
                        productIndex < category.itemIds.length - 1
                          ? () => onMoveProduct(productIndex, 'down')
                          : undefined
                      }
                    />
                  )
