...
import type { Gift, ShowToast } from '../types'
      ...
      {gift.imageUrl && gift.imageUrl.trim() !== '' && (
        <div className={imageWrapperClasses}>
          ...
          {!hideAmazonBadge &&
            gift.retailers &&
            gift.retailers.length > 0 &&
            gift.retailers.every((r) => r.name.toLowerCase().includes('amazon')) && (
              <span
                className={`absolute top-3 bg-[#232F3E] text-white text-xs font-semibold px-2 py-1 rounded shadow-md tracking-wide ${gift.trendingBadge ? 'right-3' : 'left-3'}`}
              >
                Alleen Amazon
              </span>
            )}
      ...
      >
        ...
        {onCompareToggle && !isReadOnly && !isEmbedded && (
          <div className="mb-3 flex items-center gap-2">
            ...
            />
            <label
              htmlFor={`compare-${gift.productName}`}
        ...
        <h3
          className={`font-display text-xl font-bold text-primary ${candidateVariant ? 'text-center' : ''}`}
        >
          {gift.productName}
        </h3>
        {gift.priceRange && (
          <p
            className={`mt-1 font-bold text-primary text-lg ${candidateVariant ? 'text-center' : ''}`}
          >
            {gift.priceRange}
          </p>
        )}
        ...
        {gift.story && (
          <p
            className={`mt-3 text-sm text-gray-500 italic ${candidateVariant ? 'text-center' : ''}`}
          >
            {gift.story}
          </p>
        )}
        ...
        {!isReadOnly && !isEmbedded && (
          <div className="mt-6 pt-4 border-t border-gray-200">
            <SocialShare item={gift} type="gift" variant="compact" className="justify-center" />
          </div>
        )}
      </div>
    </>
