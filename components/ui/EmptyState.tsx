...
const emptyStateVariants = cva('flex flex-col items-center justify-center text-center px-6 py-12', {
  variants: {
    variant: {
...
const EmptyState = React.forwardRef<HTMLDivElement, EmptyStateProps>(
  ({ className, variant, size, icon, title, description, action, suggestions, ...props }, ref) => {
    ...
    return (
      ...
      >
        {/* Icon */}
        <div className="mb-4 flex-shrink-0">{getDefaultIcon()}</div>

        {/* Title */}
        <h3 className="text-xl font-bold text-neutral-900 mb-2">{title}</h3>

        {/* Description */}
        {description && <p className="text-base text-neutral-600 mb-6 max-w-md">{description}</p>}
