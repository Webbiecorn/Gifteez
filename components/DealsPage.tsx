...
const DealCard: React.FC<{ item: DealItem }> = ({ item }) => (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col group transition-transform duration-300 hover:-translate-y-1">
        <div className="overflow-hidden">
            <ImageWithFallback src={item.imageUrl} alt={item.name} className="w-full h-56 object-cover group-hover:scale-105 transition-transform duration-300" />
        </div>
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-20">
        ...
        <section className="animate-fade-in">
          <div className="bg-white rounded-lg shadow-2xl overflow-hidden md:flex items-center">
            <div className="md:w-1/2 overflow-hidden">
                <ImageWithFallback src={dealOfTheWeek.imageUrl} alt={dealOfTheWeek.name} className="w-full h-64 md:h-full object-cover transition-transform duration-300 hover:scale-105"/>
            </div>
        ...
        <section className="animate-fade-in-up" style={{ animationDelay: '200ms' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
                {top10Deals.map((item, index) => (
                    <div key={item.id} className="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col group transition-transform duration-300 hover:-translate-y-1">
                        <div className="relative">
                            ...
                            <div className="overflow-hidden">
                                <ImageWithFallback src={item.imageUrl} alt={item.name} className="w-full h-40 object-cover group-hover:scale-105 transition-transform duration-300" />
                            </div>
                        </div>
                ...
                <AmazonTeaser items={amazonTeaserItems.slice(0, 8)} />
