...
export function GiftExplainer({
  ...
  const getConfidenceLevel = (score?: number): { label: string; variant: 'success' | 'primary' | 'secondary' } => {
  ...
  return (
    <div className={`bg-primary-50 border border-primary-200 rounded-lg p-4 ${className}`}>
      ...
      {isExpanded && (budgetFit !== undefined || occasionFit !== undefined || personaFit !== undefined || trendScore !== undefined) && (
        <div className="mt-4 pt-3 border-t border-primary-200">
          ...
          <div className="grid grid-cols-2 gap-2">
            {budgetFit !== undefined && (
              <ScoreBar label="Budget" score={budgetFit} />
            )}
            {occasionFit !== undefined && (
              <ScoreBar label="Gelegenheid" score={occasionFit} />
            )}
            {personaFit !== undefined && (
              <ScoreBar label="Persoonlijkheid" score={personaFit} />
            )}
            {trendScore !== undefined && (
              <ScoreBar label="Populariteit" score={trendScore} />
            )}
          </div>
        </div>
      )}
  ...
  return (
    <div className={`flex items-start gap-2 text-sm ${className}`}>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-primary-800 line-clamp-2">
          {explanations[0]}
        </p>
