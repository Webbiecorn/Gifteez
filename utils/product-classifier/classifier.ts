 ...
 */
function classifyOccasions(
  product: Product,
  keywords: KeywordSet['occasions']
): Occasion[] {
  const fullText = createSearchText(product)
 ...
 */
function classifyInterests(
  product: Product,
  keywords: KeywordSet['interests']
): Interest[] {
  const fullText = createSearchText(product)
...
function applyBrandOverrides(
  ...
  if (brandKey) {
    ...
    return {
      ...
      reasons: [
        ...facets.reasons,
        `Brand override applied: ${override.reason || brandKey}`
      ]
    }
  }
...
function applySKUOverrides(
  ...
  if (override) {
    return {
      ...
      reasons: [
        ...facets.reasons,
        `SKU override applied: ${override.reason || product.id}`
      ]
    }
  }
...
function shouldExclude(product: Product, overrides: Overrides): boolean {
  ...
  if (product.brand && overrides.exclude.brands.some(
    b => normalizeText(b) === normalizeText(product.brand!)
  )) {
    return true
  }
  ...
  if (overrides.exclude.contains.some(text => 
    fullText.includes(normalizeText(text))
  )) {
    return true
  }
