...
const CategoryDetailPage = () => {
  ...
  const topProduct = products.sort((a, b) => (b.giftScore || 0) - (a.giftScore || 0))[0]
...
const FrequentlyBoughtTogether = ({ mainProduct, suggestedProducts }) => {
  ...
  return (
    <div className="rounded-2xl border-2 border-purple-200 bg-gradient-to-br from-purple-50 to-pink-50 p-6">
      <h3 className="mb-4 font-display text-2xl font-bold text-slate-900">Vaak samen gekocht</h3>
      ...
      <div className="flex items-center justify-between">
        <div className="text-2xl font-bold text-slate-900">Totaal: €{totalPrice.toFixed(2)}</div>
    ...
    <div className="space-y-2">
      ...
      {viewCount > 10 && (
        <div className="flex items-center gap-2 rounded-lg bg-orange-50 px-3 py-2 text-sm">
          ...
          <span className="font-semibold text-orange-900">{viewCount} mensen bekijken dit nu</span>
        </div>
      )}
