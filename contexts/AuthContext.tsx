...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const mergeGuestFavorites = (user: User): User => {
    try {
      ...
      if (guestFavorites.length > 0) {
        ...
        const uniqueNewFavorites = guestFavorites.filter(
          (gf) => !userFavoriteNames.has(gf.productName)
        )
  ...
  const toggleFavorite = (gift: Gift) => {
    ...
    if (firebaseEnabled && db) {
      ...
      const isCurrentlyFavorite = currentUser.favorites.some(
        (fav) => fav.productName === gift.productName
      )
    ...
    const isCurrentlyFavorite = users[userIndex].favorites.some(
      (fav) => fav.productName === gift.productName
    )
    const updatedFavorites = isCurrentlyFavorite
  ...
  const updateProfile = async (profileToUpdate: GiftProfile) => {
    ...
    const updatedProfiles = currentUser.profiles.map((p) =>
      p.id === profileToUpdate.id ? profileToUpdate : p
    )
