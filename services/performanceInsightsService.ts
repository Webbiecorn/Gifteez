...
export class PerformanceInsightsService {
  ...
  private static queueEvent(event: PendingEvent): void {
    ...
    if (this.pendingEvents.length === 1) {
      console.log(
        '[PerformanceInsights] Batching enabled - events will be flushed in',
        this.FLUSH_INTERVAL / 1000,
        'seconds or when batch reaches',
        this.BATCH_SIZE
      )
    }
    ...
    if (this.pendingEvents.length >= this.BATCH_SIZE) {
      ...
      console.log(
        '[PerformanceInsights] Batch full, flushing',
    ...
    try {
      ...
      for (const [productId, stats] of productStats.entries()) {
        ...
        if (days === 7) {
          ...
          ctr30d =
            metrics30d.impressions > 0 ? (metrics30d.clicks / metrics30d.impressions) * 100 : 0
        }
  ...
  static async getAllMetrics(): Promise<ProductMetrics[]> {
    ...
    try {
      const q = query(collection(db, COLLECTION_METRICS), orderBy('clicks', 'desc'), limit(100))
