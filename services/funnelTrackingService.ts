...
export const FUNNEL_DEFINITIONS = {
  ...
  deals_flow: {
    name: 'Deals Flow',
    steps: ['view_deals_page', 'view_deal', 'click_affiliate', 'outbound'],
  },
  category_flow: {
    name: 'Category Flow',
    steps: ['view_category', 'view_product', 'click_affiliate', 'outbound'],
  },
  blog_flow: {
    name: 'Blog Flow',
    steps: ['view_blog_post', 'click_product_link', 'click_affiliate', 'outbound'],
  },
}
...
export function completeStep(funnelName: string, stepName: string): void {
  ...
  console.log(
    `[FunnelTracking] Completed step: ${stepName} (${stepNumber}/${funnelDef.steps.length}) in ${timeFromPrevious}ms`
  )
...
export function getFunnelMetrics(funnelName: string): FunnelMetrics | null {
  ...
  const stepMetrics: StepMetrics[] = funnelDef.steps.map((stepName, index) => {
    ...
    const dropOff =
      stepNumber < funnelDef.steps.length
        ? reached -
          sessions.filter((s) => s.steps.some((step) => step.stepNumber > stepNumber)).length
        : 0
    ...
    const averageTime =
      stepTimes.length > 0 ? stepTimes.reduce((sum, time) => sum + time, 0) / stepTimes.length : 0
