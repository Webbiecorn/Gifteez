...
import { SemanticProfile, calculateSemanticSimilarity } from './semanticLabelService';
...
export const DEFAULT_WEIGHTS: ScoringWeights = {
  ...
  personaFit: 0.30,     // 30% - Personality/interest fit
  trendScore: 0.10      // 10% - Trending/popular items
};
 ...
 */
export function calculateOccasionFit(
  gift: Gift,
  occasion: string,
  recipient: string
): number {
  ...
  if (occasionLower.includes('valentijn') || occasionLower.includes('valentine')) {
    if (giftText.includes('romantic') || giftText.includes('romantisch') || 
        giftText.includes('love') || giftText.includes('liefde')) {
      score += 0.3;
    }
  ...
  if (occasionLower.includes('kerst') || occasionLower.includes('christmas')) {
    if (giftText.includes('kerst') || giftText.includes('christmas') || 
        giftText.includes('winter') || giftText.includes('holiday')) {
      score += 0.3;
    }
  }
  ...
  if (occasionLower.includes('verjaardag') || occasionLower.includes('birthday')) {
    ...
    if (giftText.includes('verjaardag') || giftText.includes('birthday') || 
        giftText.includes('feest') || giftText.includes('party')) {
      score += 0.2;
    }
  }
  
  if (occasionLower.includes('jubileum') || occasionLower.includes('anniversary')) {
    if (giftText.includes('jubileum') || giftText.includes('anniversary') ||
        giftText.includes('luxury') || giftText.includes('luxe') ||
        giftText.includes('premium') || giftText.includes('exclusive')) {
      score += 0.3;
    }
  }
  ...
  if (recipientLower === 'partner') {
    if (giftText.includes('romantic') || giftText.includes('couple') || 
        giftText.includes('together') || giftText.includes('samen')) {
      score += 0.15;
    }
  }
  
  if (recipientLower === 'collega') {
    if (giftText.includes('professional') || giftText.includes('office') || 
        giftText.includes('kantoor') || giftText.includes('werk')) {
      score += 0.15;
    }
  }
  
  if (recipientLower === 'kind' || recipientLower === 'child') {
    if (giftText.includes('kind') || giftText.includes('child') || 
        giftText.includes('toy') || giftText.includes('speelgoed')) {
      score += 0.2;
    }
  }
