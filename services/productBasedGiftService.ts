...
import type { Gift, GiftSearchParams, Retailer } from '../types'
...
export class ProductBasedGiftService {
  ...
  private static filterProductsBySearch(products: any[], searchParams: GiftSearchParams): any[] {
    ...
    const filteredProducts = products.filter((product) => {
      ...
      const isCoolblueProduct =
        product.source === 'coolblue' ||
        (product.url && product.url.includes('coolblue')) ||
        (product.affiliateLink && product.affiliateLink.includes('coolblue'))

      const isSustainablePartner =
        product.source === 'shop-like-you-give-a-damn' ||
      ...
      if (searchTerms.length > 0) {
        ...
        const hasDirectMatch = searchTerms.some((term) => productText.includes(term || ''))

        const hasRelevanceMatch =
          this.isRelevantForRecipient(productText, recipient) ||
  ...
  private static calculateRelevanceScore(product: any, searchParams: GiftSearchParams): number {
    ...
    const isSlygadProduct =
      product.source === 'shop-like-you-give-a-damn' ||
      (product.url && product.url.toLowerCase().includes('shoplikeyougiveadamn')) ||
      (product.affiliateLink &&
        product.affiliateLink.toLowerCase().includes('shoplikeyougiveadamn')) ||
      (product.merchant && product.merchant.toLowerCase().includes('shop like you give a damn'))
  ...
  private static convertProductsToGifts(products: any[], searchParams: GiftSearchParams): Gift[] {
    return products.map((product) => {
      ...
      return {
        ...
        relevanceScore:
          typeof product.relevanceScore === 'number' ? product.relevanceScore : undefined,
      } as Gift
    })
  }
  ...
  private static resolveRetailerName(product: any): string {
    ...
    if (
      ...
      merchant.includes('shop like you give a damn') ||
      link.includes('shoplikeyougiveadamn')
    ) {
  ...
  private static generateGiftDescription(product: any, searchParams: GiftSearchParams): string {
    let baseDescription =
      product.description || product.shortDescription || product.name || 'Geweldig cadeau'
    ...
    if (
      !lowerDescription.includes(recipientInDutch.toLowerCase()) &&
   ...
   */
  private static determineGiftType(
    product: any
  ): 'physical' | 'experience' | 'digital' | 'subscription' {
    ...
    if (
      ...
      category.includes('ervaring') ||
      category.includes('cursus') ||
