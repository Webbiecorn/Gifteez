import { User, UserPreferences, NotificationSettings, Gift } from '../types';
...
export class UserService {
  ...
  static mergeFavorites(localFavorites: Gift[], cloudFavorites: Gift[]): Gift[] {
    ...
    localFavorites.forEach(localGift => {
      const exists = cloudFavorites.some(cloudGift => 
        cloudGift.productName === localGift.productName
      );
    ...
    return merged.filter((gift, index, array) =>
      index === array.findIndex(g => g.productName === gift.productName)
    );
  }
