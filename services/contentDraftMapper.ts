...
const parseParagraphHtml = (html: string): { style: ParagraphStyle; text: string } => {
  ...
  if (/^<\s*blockquote/i.test(trimmed)) {
    const inner = trimmed.replace(/^<\s*blockquote[^>]*>/i, '').replace(/<\s*\/\s*blockquote\s*>$/i, '');
...
const linesToListHtml = (lines: string[], ordered: boolean): string => {
  ...
  const listClass = ordered ? 'list-decimal pl-5 space-y-1 text-gray-700' : 'list-disc pl-5 space-y-1 text-gray-700';
  const items = lines
    .map((line) => line.trim())
    .filter(Boolean)
...
const formatParagraphDraft = (block: ParagraphBlockDraft): string => {
  ...
  switch (block.style) {
    case 'paragraph': {
      ...
      return lines.map((segment) => `<p>${escapeHtml(segment).replace(/\n/g, '<br />')}</p>`).join('');
    }
    case 'bullets': {
...
const draftGiftToGift = (draft: GiftDraft): Gift => {
  const retailers = draft.retailers
    .map((retailer) => ({ name: retailer.name.trim(), affiliateLink: retailer.affiliateLink.trim() }))
...
const draftToBlock = (draft: ContentBlockDraft): ContentBlock | null => {
  switch (draft.type) {
    ...
    case 'verdict': {
      ...
      return { type: 'verdict', title: draft.title.trim() || 'Ons oordeel', content: draft.text.trim() };
    }
    case 'unsupported':
  ...
  switch (block.type) {
    ...
    case 'image': {
      ...
      const figure = image.href
        ...
        : img;
      const caption = image.caption ? `<figcaption style="text-align:center;color:#4b5563;font-size:0.9rem;margin-top:0.5rem;">${escapeHtml(image.caption)}</figcaption>` : '';
    ...
    case 'faq': {
      ...
      const items = faq.items
        .map(
          (item) => `<div style="margin-bottom:1rem;border-bottom:1px solid #e5e7eb;padding-bottom:1rem;">
  ...
  draftsToContentBlocks(drafts)
    .map((block) => {
      switch (block.type) {
        ...
        case 'faq':
          return (block as FAQBlock).items.map((item) => `${item.question} ${item.answer}`).join(' ');
        case 'verdict':
