...
const normaliseDealItem = (rawDeal: any): DealItem | null => {
  ...
  const imageCandidate =
    typeof rawDeal.imageUrl === 'string'
      ? rawDeal.imageUrl
      : typeof rawDeal.image === 'string'
        ? rawDeal.image
        : null
  const imageUrl =
    imageCandidate && imageCandidate.trim().length ? imageCandidate : DEFAULT_PRODUCT_PLACEHOLDER
  ...
  const affiliateLink =
    typeof rawDeal.affiliateLink === 'string' ? rawDeal.affiliateLink.trim() : ''
...
const normaliseEntry = (raw: any): PinnedDealEntry | null => {
  ...
  const pinnedAt =
    typeof raw?.pinnedAt === 'number' && Number.isFinite(raw.pinnedAt) ? raw.pinnedAt : Date.now()
