...
const buildPartnerGroups = (gifts: Gift[], usedNames: Set<string>): PartnerGroups => {
  ...
  gifts.forEach((gift) => {
    ...
    if (
      primaryRetailer.includes('shop like you give a damn') ||
      primaryRetailer.includes('slygad')
    ) {
...
const selectPartnerGifts = (
  ...
  const guaranteedSlygadCount = Math.min(MIN_SLYGAD_RESULTS, slygadRanking.length, remaining)
