...
const normaliseProduct = (raw: any): CoolblueProduct => {
  ...
  const parsedOriginalPrice = typeof raw?.originalPrice === 'number'
    ? raw.originalPrice
  ...
  const product: CoolblueProduct = {
    ...
    name: typeof raw?.name === 'string' ? raw.name.trim() : String(raw?.title ?? 'Onbekend product'),
    ...
    category: typeof raw?.category === 'string' && raw.category.trim().length ? raw.category.trim() : undefined,
...
export class CoolblueFeedService {
  ...
  static generateBlogPostTemplate(product: CoolblueProduct): BlogPostData {
    ...
    sections.push('Pas deze blogpost verder aan met eigen ervaringen, foto\'s en gebruiksscenario\'s zodat hij perfect aansluit bij jouw doelgroep.');
    ...
    return {
      ...
      seo: {
        ...
        keywords: [product.name, 'cadeau', 'review', 'Coolblue', category].filter(Boolean) as string[],
  ...
  static generateMultiProductTemplate(products: CoolblueProduct[]): BlogPostData {
    ...
    const uniqueSelection = selection.filter((product, index, array) =>
    ...
    const primaryCategory = limitedSelection[0]?.category || limitedSelection[0]?.tags?.[0] || undefined;
    const heading = primaryCategory
      ? `Top ${topCount} cadeautips voor ${primaryCategory}`
      : `Top ${topCount} cadeautips vanuit de Coolblue feed`;
    const slug = BlogService.generateSlug(`${primaryCategory ?? 'cadeaus'} top ${topCount} cadeautips`);
    ...
    sections.push('Deze selectie komt rechtstreeks uit de Coolblue feed. Voeg in de editor zelf een pakkende inleiding, hero-afbeelding en persoonlijke ervaring toe voor het beste resultaat.');
    ...
    sections.push('Schrijf een persoonlijke intro, voeg een hero-afbeelding toe die het thema vangt en sluit af met een duidelijke call-to-action. Optimaliseer daarna de SEO-instellingen via de editor.');
