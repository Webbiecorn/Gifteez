...
import { RawFeedItem, NormalizationResult, NormalizedProduct } from '../../types/normalizedProduct';
  ...
  normalize(rawItem: RawFeedItem): NormalizationResult {
    try {
      ...
      if (!merchantProductId) {
        return this.createResult(false, undefined, ['Missing product_id'], undefined, true, 'No product ID');
      }
      
      const name = String(rawItem.product_name || rawItem.name || '').trim();
      if (!name) {
        return this.createResult(false, undefined, ['Missing product_name'], undefined, true, 'No product name');
      }
      ...
      const { plain: description, html: descriptionHtml } = this.sanitizeDescription(descriptionRaw, true);
      ...
      } else if (rawItem.in_stock === false || rawItem.in_stock === 'false' || rawItem.in_stock === '0') {
      ...
      const affiliateUrl = String(rawItem.deeplink_url || rawItem.deeplink || rawItem.affiliate_url || rawItem.product_url || '');
      ...
      const product: NormalizedProduct = {
        ...
        images: {
          ...
          additional: rawItem.additional_images ? 
            (Array.isArray(rawItem.additional_images) ? rawItem.additional_images : [rawItem.additional_images]) 
            : undefined
        },
        ...
        computed: {
          discountPercentage: originalPrice ? this.calculateDiscount(currentPrice, originalPrice) : undefined,
