...
export function getVariant<T>(
  ...
  const variantWeights = weights || 
    variantKeys.reduce((acc, key) => ({ ...acc, [key]: 1 / variantKeys.length }), {});
  
  // Normalize weights to sum to 1
  const totalWeight = Object.values(variantWeights).reduce((sum: number, w) => sum + (w as number), 0);
...
export function trackVariantConversion(
  ...
  const timeSinceImpression = assignment
...
function updateMetrics(
  ...
  testMetrics.overallConversionRate = testMetrics.totalImpressions > 0
