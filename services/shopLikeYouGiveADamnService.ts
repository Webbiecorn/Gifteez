...
export class ShopLikeYouGiveADamnService {
  ...
  static async loadProducts(): Promise<SLYGADProduct[]> {
    ...
    try {
      ...
      const avgScore = this.products.reduce((sum, p) => sum + (p.giftScore || 0), 0) / this.products.length;
      
      console.log(`   📊 Stats: ${featured} featured | Avg price: €${avgPrice.toFixed(2)} | Avg score: ${avgScore.toFixed(2)}/10`);
