...
export class ShopLikeYouGiveADamnService {
  ...
  private static loadingPromise: Promise<SLYGADProduct[]> | null = null
  ...
  static async loadProducts(): Promise<SLYGADProduct[]> {
    ...
    const loader = async (): Promise<SLYGADProduct[]> => {
      ...
      try {
        ...
        try {
          ...
          const response = await fetch(
            `/data/shop-like-you-give-a-damn-import-ready.json?v=${timestamp}`
          )
  ...
  static getProductsBySubcategoryName(subcategoryName: string): SLYGADProduct[] {
    return this.products.filter(
      (product) => this.detectSubcategory(product.name) === subcategoryName
    )
  }
