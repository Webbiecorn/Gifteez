...
export class SmartSuggestionsService {
  ...
  private static analyzeProductThemes(products: DealItem[]): {
    ...
    const wordFrequency = allWords.reduce(
      (acc, word) => {
  ...
  private static generateReasons(
    ...
    if (
      !isNaN(productPrice) &&
      productPrice >= themes.priceRange.min &&
      productPrice <= themes.priceRange.max
    ) {
      reasons.push(
        `Passende prijs (€${Math.round(themes.priceRange.min)}-€${Math.round(themes.priceRange.max)})`
      )
    }
